...
var Terria = function(options) {
    ...
    /**
     * Hash of properties that relate to all viewers, whether cesium or leaflet.
     *
     * @type {Object}
     */
    this.commonViewerProps = {};
    ...
    // Declare disclaimerListener as an empty function - should be overridden by the app if disclaimers are
		// required
    this.disclaimerListener = () => {};

    knockout.track(this, ['viewerMode', 'baseMap', 'baseMapName', 'fogSettings', '_initialView', 'homeView', 'pickedFeatures', 'selectedFeature', 'mapInteractionModeStack', 'configParameters', 'showTimeline', 'catalog']);
...
Terria.prototype.start = function(options) {
    ...
    return loadJson(options.configUrl).then(function(config) {
        ...
        promises.push(loadJson(cp.proxyableDomainsUrl).then(function(proxyableDomains) {
            if (proxyableDomains.proxyAllDomains) {
                corsProxy.isOpenProxy = true;
            }
            corsProxy.proxyDomains.push.apply(corsProxy.proxyDomains, proxyableDomains.proxyableDomains);
        }).otherwise(function() {
        }));
        ...
        promises.push(that.updateApplicationUrl(applicationUrl, that.urlShortener).then(function() {
