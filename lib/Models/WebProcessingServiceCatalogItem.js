...
WebProcessingServiceCatalogItem.prototype._load = function() {
    ...
    if (defined(this.parameters) && defined(this.parameterValues)) {
        ...
        var firstPolygonParameter = this.parameters.filter(function(parameter) { return parameter instanceof PolygonParameter; })[0];
        if (defined(firstPolygonParameter)) {
            var polygonCoords = this.parameterValues[firstPolygonParameter.id];
            geojson = {
                type: 'Feature',
                geometry: {
                    type: 'Polygon',
                ...
                properties: {
                    title: firstPolygonParameter.name,
                }
            };
        }
    }
...
function formatOutputValue(title, value) {
    ...
    // some wps like to output an array of strings enclosed within []
    var values;
		if (value.indexOf('[') === 0) {
    ...
    return values.reduce(function(previousValue, cValue) {
        var currentValue = cValue.trim();
        if (currentValue.indexOf("'") === 0) {
          currentValue = currentValue.substring(1,currentValue.length-1);
        }
        if (currentValue.match(/[.\/](png|jpg|jpeg|gif|svg)/i)) {
            ...
            return previousValue + '<div>' + '<a href="' + currentValue + '">' + uri.filename() + '</a>' + '</div>';
        } else {
