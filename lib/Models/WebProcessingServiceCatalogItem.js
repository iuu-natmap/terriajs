...
/*global require*/
var BBOXParameter = require('./BBOXParameter');
...
WebProcessingServiceCatalogItem.prototype._load = function() {
    ...
    if (defined(this.parameters) && defined(this.parameterValues)) {
        ...
        var firstBBOXParameter = this.parameters.filter(function(parameter) { return parameter instanceof BBOXParameter; })[0];
        if (defined(firstBBOXParameter)) {
            var rect = this.parameterValues[firstBBOXParameter.id];
            geojson = {
                type: 'Feature',
                geometry: {
                    type: 'Polygon',
    ...
    return wpsResponsePromise.then(function(xmlOrJson) {
        ...
        var outputsSection =
            ...
            (outputs || []).reduce(function(previousValue, output) {
                if (!defined(output.Data) || !defined(output.Data.LiteralData)) {
                    return previousValue;
                }
                return previousValue +
                    '<tr>' +
                        '<td style="vertical-align: middle">' + output.Title + '</td>' +
                        '<td>' + formatOutputValue(output.Title, output.Data.LiteralData) + '</td>' +
                    '</tr>';
            }, '') +
            '</table>';
...
function rectangleToPolygonArray(rect) {
    var sw = Rectangle.southwest(rect);
		var se = Rectangle.southeast(rect);
		var nw = Rectangle.northwest(rect);
		var ne = Rectangle.northeast(rect);
    return [ [ [sw.longitude, sw.latitude], [se.longitude, se.latitude], [ne.longitude, ne.latitude], [nw.longitude, nw.latitude], [sw.longitude, sw.latitude] ] ];
}
