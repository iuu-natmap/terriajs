...
var WebMapServiceCatalogItem = function(terria) {
    ...
    /**
     * Gets or sets the pickFeatures callback to be used by the WebMapServiceImageryProvider when
     * a GetFeatureInfo request is made on this layer.
     * If undefined then the default pickFeatures function on WebMapServiceImageryProvider is used.
     * @type {function}
     */
    this.pickFeatures = undefined;
    ...
    knockout.track(this, [
        ...
        'layers', 'parameters', 'getFeatureInfoFormats', 'allowFeaturePicking',
...
WebMapServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
      ...
      if (defined(myLayer) && myLayer.length > 0) {
        ...
        if (myLayer && myLayer.Style && myLayer.Style.length > 0) {
          for (var j = 0; j < myLayer.Style.length; ++j) {
            if (!defined(this.isNcWMS) && myLayer.Style[j].Name && myLayer.Style[j].Name.match(/boxfill\/rainbow/i)) {
              this.isNcWMS = true;
            }
          }
        }
    ...
    if (this.isNcWMS) {
        updateValue(this.parameters, overwrite, 'colorscalerange', "-50,50");
    }
};
...
WebMapServiceCatalogItem.prototype._createImageryProvider = function(time) {
    ...
    var wmsIP = new WebMapServiceImageryProvider({
    ...
    if (this.pickFeatures) {
        wmsIP.pickFeatures = this.pickFeatures;
    }
 ...
 */
WebMapServiceCatalogItem.prototype.refresh = function() {
    ImageryLayerCatalogItem.prototype.refresh.call(this);

    // Reapply parameters to the legend URL
    if (defined(this._legendUrl)) {
        var legendUri = new URI(this._legendUrl.url);
        for (var key in this.parameters) {
            if (this.parameters.hasOwnProperty(key)) {
                legendUri.setQuery(key, this.parameters[key]);
            }
        }
        this._legendUrl = new LegendUrl(legendUri.toString(), this._legendUrl.mimeType);
    }
};
