...
var Cesium = function(terria, viewer) {
    ...
    this._firstPoint = undefined;
		...
		// drawRectangle enabled by ALT-LEFT_CLICK 
		this._drawRectangle = new DrawExtentHelper(this.terria, this.scene, 
					function(ext) {
                ...
                if (mapInteractionModeStack[mapInteractionModeStack.length - 1].drawRectangle) {
					...
					}.bind(this), KeyboardEventModifier.ALT);
    this._drawRectangle.start();
...
Cesium.prototype.destroy = function() {
    ...
    if (defined(this._selectedFeatureSubscription)) {
		...
		if (defined(this._drawRectangle)) {
