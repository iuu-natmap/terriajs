...
var Cesium = function(terria, viewer) {
    ...
    this._outputRectangle = undefined;
    ...
    viewer.screenSpaceEventHandler.setInputAction(function(e) {
    		...
    		if (defined(mapInteractionModeStack) && mapInteractionModeStack.length > 0) {
          	if (mapInteractionModeStack[mapInteractionModeStack.length - 1].drawRectangle) {
        				this.drawRectangle(e.position); // allow the user to draw a rectangle from the current position
						} else {
...
Cesium.prototype.drawRectangle = function(screenPosition) {
    var pickRay = this.scene.camera.getPickRay(screenPosition);
    var pickPosition = this.scene.globe.pick(pickRay, this.scene);
    var pickPositionCartographic = Ellipsoid.WGS84.cartesianToCartographic(pickPosition);

		if (!defined(this._firstPoint)) {
		    this._firstPoint = pickPositionCartographic;
		    var value = getRect(pickPositionCartographic, pickPositionCartographic);
		    this._outputRectangle = value;
        // add a handler that does the MOUSE_MOVE to rubber band a rectangle
				startDrawingRect(this);

		} else {  // this is the second left click so we can stop drawing the rectangle and trigger the listener 
              // by assigning the rectangle to pickFeatures
		    stopDrawingRect(this);
        var mapInteractionModeStack = this.terria.mapInteractionModeStack;
				mapInteractionModeStack[mapInteractionModeStack.length - 1].pickedFeatures = clone(this._outputRectangle, true);
				this._outputRectangle = undefined;    // reset for next time
				this._firstPoint = undefined;    // reset for next time
		}
};
...
function startDrawingRect(cesium) {
				if (defined(cesium._dataSource)) {
				    cesium.terria.dataSources.remove(cesium._dataSource);
						cesium._dataSource = undefined;
				}
         
        cesium.viewer.screenSpaceEventHandler.setInputAction(function(movement) {
            var position = movement.endPosition;
            if (defined(position)) {
						    var pickRay = cesium.scene.camera.getPickRay(position);
								var pickPosition = cesium.scene.globe.pick(pickRay, cesium.scene);
                cesium._outputRectangle = getRect(cesium._firstPoint, Ellipsoid.WGS84.cartesianToCartographic(pickPosition));
								if (!defined(cesium._dataSource)) {
                    cesium._dataSource = new CustomDataSource('Draw Extent');
                    cesium._dataSource.entities.add({
                        name: 'Extent',
                        rectangle: {
                            coordinates: new CallbackProperty(function(date,result) {
														    if (defined(cesium._outputRectangle)) {
														         return cesium._outputRectangle;
															  }
														}, false),
                    ...
                    cesium.terria.dataSources.add(cesium._dataSource); 
							  }
            }
...
function stopDrawingRect(cesium) {
        cesium.viewer.screenSpaceEventHandler.removeInputAction(ScreenSpaceEventType.MOUSE_MOVE);
}
