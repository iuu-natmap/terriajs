...
var LeafletAltBox = require('../ThirdParty/LeafletAltBox');
...
var Leaflet = function(terria, map) {
    ...
    // if we receive altboxend (see LeafletAltBox) and we are currently
		// accepting a rectangle, then return the box as the picked feature
    map.on('altboxend', function(e) {
        ...
        if (defined(mapInteractionModeStack) && mapInteractionModeStack.length > 0) {
				    if (mapInteractionModeStack[mapInteractionModeStack.length - 1].drawRectangle && defined(e.altBoxBounds)) {
						    var b = e.altBoxBounds;
                mapInteractionModeStack[mapInteractionModeStack.length - 1].pickedFeatures = new Rectangle(b.getWest(), b.getSouth(), b.getEast(), b.getNorth());
								return true;
						}
        }
				return false;
    ...
    // click gets executed after altboxend - sigh - so don't do it if altboxend
    map.on('click', function(e) {
        var doPickFeatures = true;
				if (defined(e.originalEvent) && e.originalEvent.altKey) {
		        doPickFeatures = false;
				} 
