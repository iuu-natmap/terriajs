...
CswCatalogGroup.prototype._load = function() {
    ...
    if (this.includeWps) postDataTemplate = CswCatalogGroup.wpsGetRecordsTemplate;
...
function createItemForUri(catalogGroup, record, uri, downloadUrls, legendUrl) {
    ...
    if (defined(catalogItem)) {
        ...
        // metadata defines such a tool - at the moment if the layer is
        // awavea-intermin:...
        if (catalogItem.layers.indexOf('awavea-interim') > -1) {
          if (!defined(catalogItem.getFeatureInfoFormats)) {
            catalogItem.getFeatureInfoFormats = [];
          }
          catalogItem.getFeatureInfoFormats.push(new GetFeatureInfoFormat("text"));
          catalogItem.pickFeatures = function(tileX, tileY, tileLevel, lon, lat) {
            ...
            var submitUrl =  cleanAndProxyUrl(catalogGroup.terria, 'http://oa-gis.csiro.au/wps') + '?&version=1.0.0&request=Execute&service=WPS&Identifier=get-value-at-point';
            submitUrl += '&DataInputs=LonLatPosition=' + encodeURIComponent(JSON.stringify(geojson)) + ';layerKey=' + encodeURIComponent(catalogItem.layers);
            return loadXML(proxyUrl(catalogGroup.terria, submitUrl)).then(function(xml) {
              var resultData;
              var results = [];
