...
var createPickFeaturesUsingWebProcessingService = require('./createPickFeaturesUsingWebProcessingService');
    ...
    function loadNextPage() {
        ...
        return loadWithXhr({
            ...
            for (var i = 0; i < records.length; ++i) {
                ...
                // service records that describe WPS servers
                if (that.includeWps) {
                    var wpsuri;
                    for (var k = 0; k < uris.length; ++k) {
                      wpsuri = uris[k];
                      if (wpsuri.protocol.indexOf("OGC:WPS") > -1) {
                        // create a wps-getCapabilities process type and add it to the process tab if we have a wps uri
                        var wpsPromise = createWpsItemFromUri(that.terria, wpsuri.toString(), record.title);
                        var wpsGroup = that;
                        addProcessesToGroup(wpsPromise, wpsGroup);
                        break; // should only be one WPS url in a service record describing a WPS service
                      }
                    }
                ...
                for (var m = 0; m < uris.length; m++) {
                    ...
                    if (!excludedProtocol) {
                        ...
                        if (url.description === 'GetFeatureInfoUrl' && url.protocol.indexOf('OGC:WPS') === 0) {
                            getFeatureInfoUrl = url.toString();
                        }
                        downloadUrls.push({
                            url: url.toString(),
                ...
                for (var j = 0; j < acceptableUrls.length; ++j) {
                    ...
                    if (defined(group)) {
                        var catalogItem = createItemForUri(that, record, uri, downloadUrls, legendUrl, getFeatureInfoUrl);
...
function createWpsItemFromUri(terria, uri, title) {
    var wps = new WebProcessingServiceCatalogGroup(terria);
    wps.name = title;
    wps.url = uri;

    return wps.load().then(function() {
        return wps;
    });
}

function addProcessesToGroup(wpsPromise, wpsGroup) {
    wpsPromise.then(function(wpsItem) {
      wpsGroup.items.push(wpsItem);
    });
}
...
function createItemForUri(catalogGroup, record, uri, downloadUrls, legendUrl, getFeatureInfoUrl) {
    var layerName;
    if (defined(uri.name)) {
        layerName = uri.name;
    }
    ...
    if (defined(catalogItem)) {
        ...
        if (catalogItem.hasOwnProperty('layers') && defined(layerName)) {
            catalogItem.layers = layerName;
				...
				/* Testing
				if (catalogItem.layers.indexOf('awavea-interim') > -1) {
				  getFeatureInfoUrl = 'http://oa-gis.csiro.au/wps?version=1.0.0&request=Execute&service=WPS&Identifier=get-value-at-point&DataInputs=LonLatPosition={geojson_point};layerKey={layer}';
				}
				*/

        // now override pickFeatures with our own pickFeatures if the 
        // metadata defines a getFeatureInfoUrl with protocol WPS
        if (defined(getFeatureInfoUrl)) {
				  createPickFeaturesUsingWebProcessingService(catalogGroup.terria, catalogItem, getFeatureInfoUrl);
				}
    }
